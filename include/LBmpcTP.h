...
void LBmpcTP<Type, _n, _m, _N, _nSt, _nInp, _nF_xTheta, _pos_omega> :: compResiduals()
{
	...
	{
		r_H.template segment<_n>(_m+(i-1)*offset) = 2*Q_tilde*z.template segment<_n>(_m+(i-1)*offset) + q_tilde_vec[i-1] +
	...
	{
		r_H.template segment<_n>(_m+(_N-1)*offset+_n) = Fx_transp[_N-1]*lambda.template segment<_nSt>(_nInp+(_N-1)*(_nInp+_nSt)) + nu.template segment<_n>((_N-1)*offset1+_n);
	}
	r_H.template segment<_m>(_m+(_N-1)*offset+_n+_n) =  F_theta_transp*lambda.template segment<_nF_xTheta>(_N*(_nSt+_nInp));
	r_H = -r_H;
...
void LBmpcTP<Type, _n, _m, _N, _nSt, _nInp, _nF_xTheta, _pos_omega> :: compPhi()
{
	...
	for (int i=0 ; i <= _N-1 ; i++)
	{
		d_diagU[i].diagonal() = (lambda.template segment<_nInp>(i*(_nInp + _nSt))).cwiseQuotient(slack.template segment<_nInp>(i*(_nInp + _nSt)));
		d_diagX[i].diagonal() = (lambda.template segment<_nSt>(_nInp+i*(_nInp + _nSt))).cwiseQuotient(slack.template segment<_nSt>(_nInp+i*(_nInp + _nSt)));
	}
	d_diagTheta.diagonal() = (lambda.template segment<_nF_xTheta>(_N*(_nInp + _nSt))).cwiseQuotient(slack.template segment<_nF_xTheta>(_N*(_nInp + _nSt)));
	...
	for (int i=1; i <= _N-1; i++)
	{
		if (i != _pos_omega)
		{
			Rho[i-1] = 2*Q_bar +  Fx_transp[i-1]*d_diagX[i-1]*Fx[i-1]  + Fu_bar_transp[i]*d_diagU[i]*Fu_bar[i]  + reg * eyeN;
		}
		else	// i == _pos_omega
		{
			Rho[_pos_omega-1] = 2*Q_bar + ( Fx_transp[_pos_omega-1]*d_diagX[_pos_omega-1]*Fx[i-1]  + Fu_bar_transp[_pos_omega]*d_diagU[_pos_omega]*Fu_bar[_pos_omega] 
				+ F_xTheta_transp * d_diagTheta*F_xTheta)	+ reg * eyeN;
		}
	...
	if (_pos_omega == _N)
	{
		Rho[_N-1] = ( Fx_transp[_N-1]*d_diagX[_N-1]*Fx[_N-1] + F_xTheta_transp*d_diagTheta*F_xTheta ) + reg * eyeN;
	}
...
void LBmpcTP<Type, _n, _m, _N, _nSt, _nInp, _nF_xTheta, _pos_omega> :: compBeta()
{
	...
	for (int i=1; i<= _N-1; i++)
	{
		...
		if (i != _pos_omega)	
		{	
			r_d_bar.template segment<_n>(_m+(i-1)*offset+_n) = -r_H.template segment<_n>(_m+(i-1)*offset+_n) - Fx_transp[i-1]*tmp.template segment<_nSt>(_nInp+(i-1)*(_nInp+_nSt)) - Fu_bar_transp[i]*tmp.template segment<_nInp>(i*(_nInp+_nSt));
		}
		else	// must add the additional term: F_xTheta'*d(.)
		{
		...
		r_d_bar.template segment<_m>(_m+(i-1)*offset+_n+_n) = -r_H.template segment<_m>(_m+(i-1)*offset+_n+_n) - Fu_transp[i]*tmp.template segment<_nInp>(i*(_nInp+_nSt));
	...
	if(_pos_omega == _N)
	{
		r_d_bar.template segment<_n>(_m+(_N-1)*offset+_n) = -r_H.template segment<_n>(_m+(_N-1)*offset+_n) - Fx_transp[_N-1]*tmp.template segment<_nSt>(_nInp+(_N-1)*(_nInp+_nSt)) - F_xTheta_transp*tmp.template segment<_nF_xTheta>(_N*(_nInp+_nSt));
	}
	else	//standard
	{
		r_d_bar.template segment<_n>(_m+(_N-1)*offset+_n) = -r_H.template segment<_n>(_m+(_N-1)*offset+_n) - Fx_transp[_N-1]*tmp.template segment<_nSt>(_nInp+(_N-1)*(_nInp+_nSt)) ;
	}
	r_d_bar.template segment<_m>(_m+(_N-1)*offset+_n+_n) = -r_H.template segment<_m>(_m+(_N-1)*offset+_n+_n) - F_theta_transp*tmp.template segment<_nF_xTheta>(_N*(_nSt+_nInp));
	...
	tmp2.template segment<_m>(0) = LOmicron_diag[0].matrixLLT().triangularView<Lower>().solve(r_d_bar.template segment<_m>(0) );
	for (int i = 1; i <= _N-1; i++)
	{
		tmp2.template segment<_n>(_m+(i-1)*offset) = LPi_diag[i-1].matrixLLT().triangularView<Lower>().solve(r_d_bar.template segment<_n>(_m+(i-1)*offset));
		tmp2.template segment<_n>(_m+(i-1)*offset+_n) = LRho_diag[i-1].matrixLLT().triangularView<Lower>().solve(r_d_bar.template segment<_n>(_m+(i-1)*offset+_n));
		tmp2.template segment<_m>(_m+(i-1)*offset+_n+_n) = LOmicron_diag[i].matrixLLT().triangularView<Lower>().solve(r_d_bar.template segment<_m>(_m+(i-1)*offset+_n+_n) - LSigma_offDiag[i-1]*tmp2.template segment<_n>(_m+(i-1)*offset+_n));
	}
	if (_pos_omega == _N)
	{
		...
		tmp2.template segment<_m>(_m+(_N-1)*offset+_n+_n) = LOmicron_diag[_N].matrixLLT().triangularView<Lower>().solve(r_d_bar.template segment<_m>(_m+(_N-1)*offset+_n+_n) - LSigma_offDiag[_N-1]*tmp2.template segment<_n>(_m+(_N-1)*offset+_n));
	}
...
{	
	...
	tmp.template segment<_m>(0) = -r_d_bar.template segment<_m>(0) + Bm_transp*dnu.template segment<_n>(0) + B_transp*dnu.template segment<_n>(_n);
	
	for (int i=1; i<= _N-1; i++)
	{
		tmp.template segment<_n>(_m+(i-1)*offset) = -r_d_bar.template segment<_n>(_m+(i-1)*offset) - dnu.template segment<_n>(2*(i-1)*_n) + Am_tilde_transp*dnu.template segment<_n>(2*(i-1)*_n+_n+_n) ;
		tmp.template segment<_n>(_m+(i-1)*offset+_n) = -r_d_bar.template segment<_n>(_m+(i-1)*offset+_n) - dnu.template segment<_n>(2*(i-1)*_n+_n) + Bm_bar_transp*dnu.template segment<_n>(2*(i-1)*_n+_n+_n) + A_bar_transp * dnu.template segment<_n>(2*(i-1)*_n+_n+_n+_n);
		tmp.template segment<_m>(_m+(i-1)*offset+_n+_n) = -r_d_bar.template segment<_m>(_m+(i-1)*offset+_n+_n) + Bm_transp*dnu.template segment<_n>(2*(i-1)*_n+_n+_n) + B_transp*dnu.template segment<_n>(2*(i-1)*_n+_n+_n+_n);
	}
	
	tmp.template segment<_n>(_m+(_N-1)*offset) = -r_d_bar.template segment<_n>(_m+(_N-1)*offset) - dnu.template segment<_n>(2*(_N-1)*_n);
	tmp.template segment<_n>(_m+(_N-1)*offset+_n) = -r_d_bar.template segment<_n>(_m+(_N-1)*offset+_n) - dnu.template segment<_n>(2*(_N-1)*_n+_n);
	tmp.template segment<_m>(_m+(_N-1)*offset+_n+_n) = -r_d_bar.template segment<_m>(_m+(_N-1)*offset+_n+_n);
