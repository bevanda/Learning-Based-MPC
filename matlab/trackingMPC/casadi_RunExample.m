...
iterations = 100;
delta= 1; % discrete system, already sampled
...
true_refHistory = xs;
opt_var=MX.sym('y',N*m+(N+1)*n);
obj=cost_fun(N, opt_var, LAMBDA*opt_var(end-m+1:end), PSI*opt_var(end-m+1:end), Q, R, P,n,m,delta);
con=nl_cons(N, delta, opt_var, LAMBDA*opt_var(end-m+1:end), P, alpha,n,m);
nlp = struct('x', opt_var, 'f', obj, 'g', con);
