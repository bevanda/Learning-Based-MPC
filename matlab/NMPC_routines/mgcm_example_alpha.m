...
% -------------------------------------------------------------------------
    ...
    A_continuous = [1.12500000000000,1,0,0;1,-0.444444237222174,-1.29903810567666,0;0,0,0,1;0,0,-1000,-44.7213595499958];   % Coefficient matrix A
    B_continuous = [0;0;0;1000];                            % Coefficient matrix B

    Q = 1.0*eye(dimension_x, dimension_x);                  % Weighting matrix Q
    R = 1.0*eye(dimension_u, dimension_u);                  % Weighting matrix R
    T = 1000;
...
% [0.150000000000000,1.28750000000000,1.15470000000000,0]
    x1 = 0.15;                                               % Initial values
    x2 = 1.2875;
    x3 = 1.1547;
    x4 = 0.0;
    ...
    iplot = 1;             % Switch for plotting, 1-plotting, 0-no plots
                                                           
    time = 10;                                                         
    N_plot = time/T;                                        % Application horizon / iterations
    Nmvals = [1 2];                           % Values for m-step feedback
    
%     K = -lqr(A_continuous, B_continuous, Q, R)
    K=-place(A_continuous,B_continuous,[-22.5,-21.5,-1.0,-0.1])
    A_continuous = A_continuous+B_continuous*K;
% -------------------------------------------------------------------------
% Setting up grid
% -------------------------------------------------------------------------
    x_t_akt = [];
