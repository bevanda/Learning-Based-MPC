...
for k = 1:(iterations)      
    ...
    COSTFUN = @(var) costLMPC(reshape(var(1:end-m),m,N),reshape(var(end-m+1:end),m,1),...
        x_eq,x_eq_ref,N,reshape(var(1:m),m,1),Q,R,P,T,Kstabil,LAMBDA,PSI);
    CONSFUN = @(var) constraintsLMPC(reshape(var(1:end-m),m,N),reshape(var(end-m+1:end),m,1),...
        x_eq,N,Kstabil,F_x,h_x,F_u,h_u,F_w_N,h_w_N);
    ...
    [x, u] = transitionTrue(x,c,x_w,u_w,Kstabil,dT); % plant  
