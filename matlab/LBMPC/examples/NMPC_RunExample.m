...
F_u = [eye(m); -eye(m)]; h_u = [umax-u_w-shrink; -umin+u_w+shrink];
...
for k = 1:(iterations)      
    ...
    COSTFUN = @(var) costNMPC(reshape(var(1:end-m),m,N),reshape(var(end-m+1:end),m,1),...
        x_eq,x_eq_ref,N,reshape(var(1:m),m,1),Q,R,P,T,Kstabil,x_w,u_w,LAMBDA,PSI,dT);
    CONSFUN = @(var) constraintsNMPC(reshape(var(1:end-m),m,N),reshape(var(end-m+1:end),m,1),...
        x_eq,N,x_w,u_w,Kstabil,F_x,h_x,F_u,h_u,F_w_N,h_w_N,dT);
