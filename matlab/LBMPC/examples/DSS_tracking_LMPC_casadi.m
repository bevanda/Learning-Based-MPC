...
obj=costfunction(N,A,B, y,init_x, x_eq, u_eq,  Q, R, P,T, LAMBDA,PSI, n,m,delta);
con=nonlinearconstraints(N,A,B, delta,x_eq,u_eq,y, init_x,  n,m,F_x,h_x, F_u,h_u, F_w_N, h_w_N);
...
function cost = costfunction(N, A,B, y, init_x, x_eq, u_eq, Q, R, P,T,LAMBDA,PSI, n,m,delta)
    ...
    for k=1:N
        ...
        x_k = x_eq+nominal_dynamics(x_k-x_eq, u_k-u_eq,A,B);
    end
    cost = cost + terminalcosts(x_k, x_eq, x_art+x_eq, P,T);
...
function [con] = nonlinearconstraints(N,A,B, delta,x_eq,u_eq, y, init_x,n,m,state_F,state_h, in_F,in_h, F_w_N, h_w_N) 
   ...
   % constraints along prediction horizon
    for k=1:N
        u_k=u((k-1)*m+1:k*m);
        x_new = x_eq+nominal_dynamics(x_k-x_eq, u_k-u_eq,A,B);   
        % other constraints
