function [sysHistory,art_refHistory,true_refHistory]...
          =ocpLMPC(x,x_wp_init,x_wp,x_wp_ref,u_wp,...
...
%==========================================================================
% Solving the Linear Model Predictive optimal control problem 
% at every timestep
%==========================================================================
for k = 1:(iterations)      
    ...
    % Solve the OCP
    COSTFUN = @(var) costLMPC(reshape(var(1:end-m),m,N),reshape(var(end-m+1:end),m,1),...
        dx,x_wp_ref,N,reshape(var(1:m),m,1),Q,R,P,T,Kstabil,LAMBDA,PSI);
    CONSFUN = @(var) constraintsLMPC(reshape(var(1:end-m),m,N),reshape(var(end-m+1:end),m,1),...
        dx,N,Kstabil,F_x,h_x,F_u,h_u,F_w_N,h_w_N);
